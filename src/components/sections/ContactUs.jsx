...
export const ContactUs = () => {
  ...
  const handleSubmit = () => {
    ...
    // Crear el body del email
    const subject = `Nuevo mensaje de contacto - ${formData.name}`;
    const body = `
Nombre: ${formData.name}
Email: ${formData.email}
Teléfono: ${formData.number || 'No proporcionado'}
...
Enviado desde el formulario de contacto del sitio web.
    ...
    // Crear enlace mailto
    const mailtoLink = `mailto:info@drdiegohernandez.com.co?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
          ...
          >
            <div className="flex flex-col gap-5">
              ...
              <button
                ...
                className="text-white py-3 px-6 rounded-lg font-semibold text-base transition-all duration-200 hover:opacity-90 mt-4 flex items-center justify-center gap-2"
                style={{ 
              ...
              {status === 'success' && (
                <div className="flex items-center gap-2 p-3 rounded-lg" style={{ backgroundColor: '#d4edda', color: '#155724' }}>
                  <Check size={16} />
                  <span className="text-sm">Email client opened! Please send the message from your email app.</span>
                </div>
              )}
